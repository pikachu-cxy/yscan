...
`
	...
	flag.BoolVar(&poc, "poc", false, "是否进行自动poc探测,默认不探测")
	//flag.StringVar(&subdomain, "domain", false, "是否爆破子域名，默认不爆破")
	flag.StringVar(&searchPoc, "s", "", "指定poc探测，例：-s shiro,ruoyi")
	flag.Parse()
	...
	if filePath == "" && host != "" {
		...
		Format.Choose(host, port, true, dict, outPut, path, poc, searchPoc)
	}
	if filePath != "" && host == "" {
		File.CreateFile(outPut)
